...
export default function NewFixedExpensePage(){
  ...
  // busca dados da URL
  const [data, setData] = useState<FixedExpense | null>(null);
  const token = typeof window !== "undefined" ? localStorage.getItem("token") ?? "" : "";
  const { error, updateFixedExpenses } = useFixedExpense(token);  
  ...
  const returnToPrincipal = () => router.push("/fixed_expense");

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if(!id) return;
    const success = await updateFixedExpenses(
      { balance, 
        description, 
        amount, 
          ...
          (
            ...
            <form onSubmit={handleSubmit}>
                ...
                <Input
                  ...
                  value={String(dueday)}
                  onChange={(e) => setDueDay(String(e.target.value))}
